...
Now we'll take a look at resampling.
...
$$
M_N^{-1} \sum_{i=1}^{M_N}E\left[ \left| f(\xi^{N,i}) \right| \mathbb{1}\left( \left| f(\xi^{N,i}) \right| \ge \epsilon M_N \right) \bigg\rvert \mathcal{F}^N \right] \le M_N^{-1} \sum_{i=1}^{M_N}E\left[ \left| f(\xi^{N,i}) \right| \mathbb{1}\left( \left| f(\xi^{N,i}) \right| \ge C \right) \bigg\rvert \mathcal{F}^N \right] \overset{\text{p}}{\to} \mu(|f|1\{|f| \ge C\}).
$$
...
This theorem is a little more interpretable because we are using the notation in the SIR algorithm. Here it will be more clear which samples are resampled, and which are not. 
...
Then $\{(\tilde{\xi}^{N,i}, 1)\}_{1 \le i \le \tilde{M}_M}$ is (still) consistent for $(\mu, \mathsf{C})$.


## Theorem 9.2.9: proof

TODO: pick up here (page 17). Also consider squeezing more material into the first two days of 9.2.
