...
$$
p(x_{0:t}, y_{0:t}) = n(x_0)g(y_0 | x_0)q(x_1 \mid x_0)g(y_1 \mid x_1) \times \cdots \times q(x_1 \mid x_0)g(y_1 \mid x_1)
$$
- not 
$$
p(x_{0:t} \mid y_{0:t}) = \frac{p(x_{0:t}, y_{0:t})}{p(y_{0:t})}
$$
...
versus
$$
\phi_{0:k+1 | k}(f) = \int\cdots \int f(x_{0:k+1})\phi_{0:k|k}(dx_{0:k}) \underbrace{\frac{L_{0:k}}{L_{0:k+1}}Q(x_k, dx_{k+1})g(x_{k+1})}_{T^u_k}
$$
where $f$ is any measurable, real-valued function. Note $T^u_k(x,\mathsf{X}) \neq 1$, so this kernel is not normalized (hence the $u$).

We target $\phi_{0:k+1 | k}$, but $\frac{L_{0:k}}{L_{0:k+1}}$ is typically intractable.
...
### Targets (think posteriors):
  - $\phi_{0 |0}$, $\phi_{0:1 | 1}$, ...
  - $p(x_0 \mid y_0)$, $p(x_{0:1} \mid y_{0:1})$, ...

### Unnormalized Targets (think "complete-data" likelihoods)
  - $\nu(dx_0)g_0(x_0)$, $\nu(dx_0)g_0(x_0)Q(x_0,dx_1)g_1(x_1)$, ...
  - $p(x_0)g(y_0 \mid x_0)$, $p(x_0)g(y_0 \mid x_0)q(x_1 \mid x_0) g(y_1 \mid x_1)$, ...

### Proposals/Instrumental Distributions
  - $\rho_0(d x_0)$, $\rho_{0:1}(d x_{0:1}) = \rho_0(d x_0)R(x_0, dx_1)$, ...
  - $r(x_0)$, $r(x_{0:1})$, ...
...
if $\phi_{0:k|k}$ is absolutely continuous with respect to $\rho_{0:k}$.
...
For $i=1,2,\ldots,N$:
  ...
  b. for $k> 0$  $\omega_{k+1}^i = \omega_{k} g_{k+1}(\xi_{k+1}^i) \frac{d Q( \xi_k^i, \cdot)}{dR(\xi^i_k, \cdot) }(\xi_{k+1}^i)$
  
Multiplicative updates.

NB: we only need to be able to evaluate from model distribution's and the proposal! Evaluation of the tricky normalizing constant has been avoided!.
